...
pub async fn find_thread_id_by_name_in_cwd(
    ...
    // Legacy fallback: preserve existing global behavior (newest global name match wins)
    // when there is no project-local hit.
    find_thread_id_by_name(codex_home, name).await
}
